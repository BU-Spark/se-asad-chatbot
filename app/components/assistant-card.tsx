...
export function AssistantCard({ assistant }: { assistant: AssistantLike }) {
    const created =
        assistant.createdAt ? new Date(assistant.createdAt as any).toLocaleDateString() : null;
        ...
        <Card>
            <CardHeader>
                <CardTitle className="flex items-center justify-between">
                    ...
                    {assistant.model && (
                        <span className="text-xs font-medium text-muted-foreground">{assistant.model}</span>
                    )}
                </CardTitle>
